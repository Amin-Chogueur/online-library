...
export default function StationeryDetails() {
  ...
  const { data: stationeryDetailsData, isLoading: stationeryDetailsLoading } =
    useQuery({
  ...
  return (
    <div className=" max-w-5xl mx-auto  text-gray-100 mb-[20px]">
      ...
      {stationeryDetailsData?.product &&
        stationeryDetailsData?.product.quantity === 0 && (
      ...
      {stationeryDetailsLoading ? (
        ...
        <>
          ...
          {stationeryDetailsData?.product && (
            <ProductDetails product={stationeryDetailsData?.product} />
          )}
        </>
      )}
    </div>
